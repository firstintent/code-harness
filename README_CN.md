...
```
Claude Code (执行) ← .claude/rules/*.md (标准)
         ↓                        ↑
    Stop hook          人工拒绝信号
         ↓                        ↑
...
选项：`--force` 覆盖已有文件，`--dashboard` 同时安装 Web 仪表盘。
...
- `.claude/rules/base-standards.md` —— 调整基础标准
...
│   ├── hooks/
│   │   ├── protect-arch.sh             # 架构约束
│   │   └── check-ownership.sh          # 多机文件所有权
│   └── rules/
│       ├── playbook.md                 # 工作流指南
│       ├── base-standards.md           # 全局质量标准
│       ├── api-quality.md              # API 标准（路径限定）
│       └── frontend-quality.md         # 前端标准（路径限定）
│
└── .harness/
    ├── tasks.md                        # 任务列表 + 认领状态
...
完整协议参见 `.claude/rules/playbook.md` 中的多机协调章节。
